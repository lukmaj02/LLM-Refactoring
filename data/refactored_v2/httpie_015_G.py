...
# condition: G
# timestamp: 2026-06-04T14:03:11
# original_cc: 10, original_mi: None
# changed_pct: 0.8452
# === END HEADER ===
def _determine_is_stream(
    processing_options: ProcessingOptions,
    message_type: Type[HTTPMessage],
    headers: HTTPHeadersDict,
) -> bool:
    """Determine if the stream should be treated as a continuous stream."""
...
def _get_raw_stream_config(is_stream: bool) -> Tuple[Type['BaseStream'], dict]:
    """Get stream class and kwargs for RawStream."""
...
def _get_encoded_stream_config(
    env: Environment,
    processing_options: ProcessingOptions,
    message_type: Type[HTTPMessage],
    prettify_groups: Any,  # Set[str]
    is_stream: bool,
) -> Tuple[Type['BaseStream'], dict]:
    """Get stream class and kwargs for EncodedStream, PrettyStream, or BufferedPrettyStream."""
...
def get_stream_type_and_kwargs(
    ...
    if not env.stdout_isatty and not prettify_groups:
        return _get_raw_stream_config(is_stream)
    else:
        return _get_encoded_stream_config(
            env,
            processing_options,
            message_type,
            prettify_groups,
