...
) -> Union[IO, bytes, 'MultipartEncoder']:
    """
    Handles special logic for zero-length files, especially stdin.
    Returns the potentially modified file object or its content as bytes.
    """
    if is_stdin(file):
...
def _prepare_file_for_upload(
    ...
    if not super_len(file):
        file = _handle_zero_length_file(
            env, file, read_event, chunked, content_length_header_value
        )
    else:
