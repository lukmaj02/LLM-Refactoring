...
def _prepare_download_state(
        ...
        initial_total_size: Optional[int]
    ) -> Optional[int]:
        """
        Helper to determine the output file, resume state, and final total size.
        Modifies `self._output_file` and `self._resumed_from`.
        """
        ...
        if not self._output_file:
            ...
            if self._resume and final_response.status_code == PARTIAL_CONTENT:
                ...
                # Not resuming, or not a partial content response.
                # Reset resume state and truncate the file.
    ...
    def start(
        ...
        try:
            initial_total_size = int(final_response.headers['Content-Length'])
        except (KeyError, ValueError, TypeError):
            initial_total_size = None

        total_size = self._prepare_download_state(
            initial_url=initial_url,
            final_response=final_response,
